...
if (PMC_debug) then
{
	diag_log format
	[
		"PMC_sendgroup_west: received _group: %1, West_ControlList: %2, West_AssignList: %3.",
		_group,
		West_ControList,
		West_AssignList
	];
...
if (!alive leader _group) exitWith
{
	...
	[
		"%1 group=dead. exiting PMC_sendgroup_west.sqf. count: %2. alive: %3",
		_group,
		count units _group,
		alive leader (_group)
	];
...
{
	if (West_ControList select _A == 0 && West_AssignList select _A == 0) exitWith
	{
		_target = PMC_targets select _A;
		_noMatchFound = false;
		if (PMC_debug) then
		{
			diag_log format["_group: %1, _A: %2, West_ControList: %3, West_AssignList: %4", _group, _A, West_ControList, West_AssignList];
		};
	};
	_A = _A + 1;
};
