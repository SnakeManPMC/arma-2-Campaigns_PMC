...
"RU_Soldier_Pilot" createUnit [getPosASL east_helostart, _heloGrp, "", 1, "SERGEANT"];
"RU_Soldier_Pilot" createUnit [getPosASL east_helostart, _heloGrp, "", 1, "CORPORAL"];

_poo = ["Mi17_Rockets_RU", "Mi24_V", "Mi24_P", "Mi24_D"];
_r1 = floor (random count _poo);
_rtmptnk = _poo select _r1;

if (PMC_debug) then
{
	player sideChat format["OPFOR helo_reinf.sqf is now running! - %1", _rtmptnk];
	diag_log format["OPFOR helo_reinf.sqf is now running! - %1", _rtmptnk];
};

_helo = _rtmptnk createVehicle getPosASL east_helostart;
...
_ran = random(1);
"RU_Soldier" createUnit [getPosASL east_helostart, _grp1, "", _ran, "SERGEANT"];
_ran = random(1);
"RU_Soldier_AT" createUnit [getPosASL east_helostart, _grp1, "", _ran, "CORPORAL"];
_ran = random(1);
"RU_Soldier_AT" createUnit [getPosASL east_helostart, _grp1, "", _ran, "CORPORAL"];
...
_ran = random(1);
"RU_Soldier" createUnit [getPosASL east_helostart, _grp2, "", _ran, "SERGEANT"];
_ran = random(1);
"RU_Soldier_AA" createUnit [getPosASL east_helostart, _grp2, "", _ran, "CORPORAL"];
_ran = random(1);
"RU_Soldier_AA" createUnit [getPosASL east_helostart, _grp2, "", _ran, "CORPORAL"];
...
if (PMC_debug) then
{
	player sideChat format["OPFOR helo_reinf.sqf target - %1", _helo_dest];
	diag_log format["OPFOR helo_reinf.sqf target - %1", _helo_dest];
};
...
if (!alive _helo) exitWith
{
	if (PMC_debug) then
	{
		player sideChat format["OPFOR helo_reinf.sqf helo entered slowdelete - %1", _helo];
		diag_log format["OPFOR helo_reinf.sqf helo entered slowdelete - %1", _helo];
	};
};
...
leader _heloGrp move (getPosASL east_helostart);

if (PMC_debug) then
{
	player sideChat format["OPFOR helo_reinf.sqf helo heading home - %1", _helo];
	diag_log format["OPFOR helo_reinf.sqf helo heading home - %1", _helo];
};
...
if (PMC_debug) then
{
	player sideChat "OPFOR helo_reinf.sqf exit";
	diag_log "OPFOR helo_reinf.sqf exit";
};
